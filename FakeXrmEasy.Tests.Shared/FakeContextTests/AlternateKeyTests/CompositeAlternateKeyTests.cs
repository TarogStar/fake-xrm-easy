...
namespace FakeXrmEasy.Tests.FakeContextTests.AlternateKeyTests
{
    ...
    public class CompositeAlternateKeyTests
    {
        ...
        [Fact]
        public void Create_Should_Handle_Money_Key_Attribute()
        {
            // Arrange
            var context = new XrmFakedContext();

            // Set up alternate key with Money attribute (e.g., creditlimit)
            context.AddAlternateKey("account", new[] { "creditlimit", "name" });

            var existingAccount = new Entity("account")
            {
                Id = Guid.NewGuid(),
                ["creditlimit"] = new Money(10000.00m),
                ["name"] = "Acme Corp"
            };
            context.Initialize(new List<Entity> { existingAccount });
            ...
            // Create a duplicate with same Money value
            var duplicateAccount = new Entity("account")
            {
                ["creditlimit"] = new Money(10000.00m),
                ["name"] = "Acme Corp"
        ...
        [Fact]
        public void Create_Should_Handle_Different_Money_As_Unique()
        {
            // Arrange
            var context = new XrmFakedContext();

            // Set up alternate key with Money attribute
            context.AddAlternateKey("account", new[] { "creditlimit", "name" });
            ...
            // Create account with different Money value - should succeed
            var newAccount = new Entity("account")
            {
                ["creditlimit"] = new Money(20000.00m), // Different amount
                ["name"] = "Acme Corp"
            };

            // Act
            var newId = service.Create(newAccount);

            // Assert - should succeed because Money value is different
            Assert.NotEqual(Guid.Empty, newId);
        }

        [Fact]
        public void Create_Should_Handle_Money_Only_Key()
        {
            // Arrange
            var context = new XrmFakedContext();

            // Set up alternate key with only Money attribute
            context.AddAlternateKey("account", "creditlimit");

            var existingAccount = new Entity("account")
            {
                Id = Guid.NewGuid(),
                ["creditlimit"] = new Money(50000.00m)
            };
            context.Initialize(new List<Entity> { existingAccount });
            ...
            var newAccount = new Entity("account")
            {
                ["creditlimit"] = new Money(75000.00m)
            };
